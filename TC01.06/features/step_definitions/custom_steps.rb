...
Then /^I am successfully login to cloud app with items recently added on the device$/ do
...
while (count <=100)
sleep 0.2
if (query("gridview id:'latestImages'").to_s.include? 'latestImages') == true
break
else
count = count + 1
 if ((query("gridview id:'latestImages'").to_s.include? 'latestImages') == false && count >= 100) then
        macro 'I take a screenshot'
        puts 'Available pictures in the device were not shown in time'
exit
    else end
end
end
